...
router.get('/meters/:meterId/readings', async (req, res, next) => {
  try {
    const data = await condoService.listMeterReadings(
      getCompanyId(req), String(req.params.meterId),
...
router.post('/meters/:meterId/readings', validateRequest(addMeterReadingSchema), async (req, res, next) => {
  try {
    const data = await condoService.addMeterReading(getCompanyId(req), String(req.params.meterId), req.body);
...
router.put('/meters/:meterId/device', validateRequest(upsertSmartDeviceSchema), async (req, res, next) => {
  try {
    const data = await condoService.upsertSmartDevice(getCompanyId(req), String(req.params.meterId), req.body);
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/meters/:meterId/sync', async (req, res, next) => {
  try {
    const data = await condoService.syncMeter(getCompanyId(req), String(req.params.meterId));
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/meters/:unitId/generate-invoice', async (req, res, next) => {
  try {
    const data = await condoService.generateUtilityInvoice(
      getCompanyId(req), String(req.params.unitId),
...
router.get('/funds/:id/transactions', async (req, res, next) => {
  try {
    const result = await condoService.listFundTransactions(
      getCompanyId(req), String(req.params.id),
...
router.post('/funds/:id/transactions', validateRequest(addFundTransactionSchema), async (req, res, next) => {
  try {
    const data = await condoService.addFundTransaction(getCompanyId(req), String(req.params.id), getUserId(req), req.body);
    res.status(201).json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.patch('/funds/transactions/:txnId/approve', async (req, res, next) => {
  try {
    const data = await condoService.approveFundTransaction(getCompanyId(req), String(req.params.txnId) as string, getUserId(req));
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.patch('/funds/transactions/:txnId/reject', async (req, res, next) => {
  try {
    ...
    const data = await condoService.rejectFundTransaction(getCompanyId(req), String(req.params.txnId) as string, getUserId(req), reason || '');
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.get('/meetings/:id', async (req, res, next) => {
  try {
    const data = await condoService.getMeetingDetail(String(req.params.id), getCompanyId(req));
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.patch('/meetings/:id/status', validateRequest(updateMeetingStatusSchema), async (req, res, next) => {
  try {
    const data = await condoService.updateMeetingStatus(String(req.params.id), getCompanyId(req), req.body);
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.get('/meetings/:id/results', async (req, res, next) => {
  try {
    const data = await condoService.getMeetingResults(String(req.params.id), getCompanyId(req));
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/meetings/:id/resolutions', validateRequest(addResolutionSchema), async (req, res, next) => {
  try {
    const data = await condoService.addResolution(getCompanyId(req), String(req.params.id), req.body);
    res.status(201).json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/meetings/:meetingId/resolutions/:resolutionId/vote', validateRequest(castVoteSchema), async (req, res, next) => {
  try {
    const data = await condoService.castVote(
      getCompanyId(req), String(req.params.meetingId), String(req.params.resolutionId), getUserId(req), req.body,
    );
    res.status(201).json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/meetings/:id/proxies', validateRequest(submitProxySchema), async (req, res, next) => {
  try {
    const data = await condoService.submitProxy(getCompanyId(req), String(req.params.id), req.body);
    res.status(201).json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/meetings/:id/send-notice', async (req, res, next) => {
  try {
    const data = await condoService.sendMeetingNotice(String(req.params.id), getCompanyId(req), getUserId(req));
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/meetings/:id/minutes', async (req, res, next) => {
  try {
    const data = await condoService.publishMinutes(String(req.params.id), getCompanyId(req), req.body);
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.patch('/bylaws/:id', async (req, res, next) => {
  try {
    const data = await condoService.updateBylaw(String(req.params.id), getCompanyId(req), req.body);
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/violations/:id/fine', validateRequest(fineViolationSchema), async (req, res, next) => {
  try {
    const data = await condoService.fineViolation(String(req.params.id), getCompanyId(req), req.body);
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/violations/:id/appeal', validateRequest(appealViolationSchema), async (req, res, next) => {
  try {
    const data = await condoService.appealViolation(String(req.params.id), getCompanyId(req), req.body);
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
...
router.post('/violations/:id/resolve', validateRequest(resolveViolationSchema), async (req, res, next) => {
  try {
    const data = await condoService.resolveViolation(String(req.params.id), getCompanyId(req), req.body);
    res.json({ success: true, data });
  } catch (e) { next(e); }
});
